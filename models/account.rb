class Account < ActiveRecord::Base
  ...
  def friend?(user)
    ...
    account_relation_count = AccountRelation.where('(uid = ? and follow_uid = ?) or (uid = ? and follow_uid = ?)', uid, user.uid, user.uid, uid).count
  ...
  def friends
    user_follow_ids = AccountRelation.where('uid = ?', uid).pluck(:follow_uid)
    each_follow_ids = AccountRelation.where(uid: user_follow_ids).where('follow_uid = ?', uid).pluck(:uid)
